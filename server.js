...
app.post('/api/upload', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.storage.from(targetBucket).upload(filePath, buffer, { contentType: `image/${ext}`, upsert: true });
    if (error) throw error;
    const { data: urlData } = supabase.storage.from(targetBucket).getPublicUrl(filePath);
    res.json({ url: urlData.publicUrl, path: filePath, bucket: targetBucket, avatarType: avatarType || 'user' });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
